  ...
  */
    ...
    const API = {
        ...
        async fetchWhatToMine() {
            // WhatTToMine has no CORS headers so we proxy it
    ...
    const Calculator = {
        ...
        miningGrossRevenue(hashrate, algorithm, liveData) {
            ...
            if (liveData && liveData.coins) {
                // Find the most prfitable coin for this algorithm
                let bestRevenue = 0;
                let bestCoin = null;
                ...
                for (const [coinKey, coinData] of Object.entries(liveData.coins)) {
                    ...
                    if (wtmAlgo === ourAlgo || this._algoMatch(wtmAlgo, ourAlgo)) {
                        // WhatToMine goves btc_revenue per unit hashrate per day
        ...
        bestMiningProfit(gpu, electricityCost, liveData, btcFees, fees) {
            ...
            // CPU mining override. Uses pre-calculated gross daily revenue
